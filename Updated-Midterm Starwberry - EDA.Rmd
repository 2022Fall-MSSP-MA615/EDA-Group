...
# View(e1)
# NEW JERSEY have no data about Value in 2019. But we can see its CWT decrease. So, we give impute this NA according to proportion.
...
```
To have a better sense of whethe the sales increase or decrease, we change the plot type into lines instead of points.
```{r echo=FALSE}
ggplot(e1m) +
  aes(x = Year, y = Value, colour = State) +
  geom_line(size = 0.5) +
...
Below is a plot that describes the sales measured in CWT for organic strawberries.
...
```
Based on the output above, California seems to be cautious about the chemicals relatively as the vast majority of the fresh strawberries sold in this country are grown there. But because of the limited information we have, we cannot conclude California has the best practice to protect 
public health. 
